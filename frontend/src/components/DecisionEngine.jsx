...
export default function DecisionEngine() {
  ...
  const handleAddOption = async (e) => {
    ...
    currentTopicData.criteria.forEach(c => { initialBlankScores[c.id] = 75; });
                ...
                <div key={index} className="bg-slate-950/80 border border-slate-800/80 p-3 rounded-xl flex justify-between items-start gap-4">
                  <div className="space-y-0.5">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[8px] font-bold px-1.5 py-0.2 rounded font-mono bg-cyan-500/10 text-cyan-400 border border-cyan-500/20">
                        LIVE GEMINI
                      </span>
                    </div>
        ...
        <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-4 shadow-xl">
          <form onSubmit={handleAddOption} className="space-y-2">
            ...
            <div className="flex gap-2">
              ...
              />
              <button type="submit" className="bg-cyan-500 hover:bg-cyan-400 text-slate-950 px-4 rounded-xl transition-all font-bold flex items-center justify-center shadow-lg">
                <Plus size={18} />
              </button>
            </div>
          </form>
              ...
              <div key={opt.id} className="bg-slate-900/60 border border-slate-800 rounded-2xl p-4 shadow-lg space-y-3.5">
                <div className="flex justify-between items-center gap-3">
                  <div>
                    ...
                    <span className="text-[9px] bg-slate-950 text-slate-400 px-2 py-0.5 rounded border border-slate-800 font-mono font-bold mt-1 inline-block">
                  ...
                  <div className="text-right flex items-center gap-2 shrink-0">
                    ...
                    <button type="button" onClick={() => setOptions(prev => prev.filter(o => o.id !== opt.id))} className="text-slate-600 hover:text-rose-400 p-1.5 rounded-lg hover:bg-rose-500/10 transition-colors">
                      <Trash2 size={14} />
                    </button>
                  </div>
                </div>
                ...
                <div className="bg-slate-950 border border-slate-800/60 rounded-xl p-3">
                  ...
                  {isItemLoading ? (
                    <p className="text-slate-500 text-xs animate-pulse py-0.5">Processing custom metric alignments...</p>
                  ) : (
